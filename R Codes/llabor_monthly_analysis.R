...
# using packages in the research room is different. the location of the packages has to be specified (lib.loc). see following code
libloc<- "D:\\Research\\Nili_Ben_Tubin_13800258_Research\\R_Packages\\3.6"
...
#monthly analysis including mityaashim------------------------------------------------------------------------------------------------------------

#create the result output
results_expanded <- monthly_regular_analysis(wrk_df,workforce = workforce_attr_inc_mit)
#bind the list into one dataframe
results_expanded <- bind_rows(results_expanded) %>% rows_order

#split the big DF to different tables by their columns, later they will be different excel sheets
to_export_expanded <- list()
to_export_expanded[["סהכ"]] <- results_expanded %>%
  select(starts_with(c("population_group","population_total"))) %>% 
  group_names

to_export_expanded[["שיעורים"]] <- results_expanded %>%
  select(contains(c("population_group","ratio"))) %>% 
  group_names

to_export_expanded[["תצפיות"]] <- results_expanded %>%
  select(starts_with(c("population_group","obs"))) %>% 
  group_names

#replace column names to hebrew
names(to_export_expanded[["סהכ"]]) <- gen_col_names_mit()
names(to_export_expanded[["תצפיות"]]) <- gen_col_names_mit()
names(to_export_expanded[["שיעורים"]]) <- gen_col_names_ratio()

#create the excel file
#add worksheet and assign data for each item in to_export_expanded list
mywb_expanded <- createWorkbook()
Map(function(dat,dat_name){
  addWorksheet(mywb_expanded,dat_name)
  writeData(mywb_expanded,dat_name,dat,startRow = 2)
},to_export_expanded,names(to_export_expanded))
#freeze panes and adjust column width for each sheet
Map(function(sheet_name){
  freezePane(mywb_expanded,sheet_name,firstCol = T)
  setColWidths(mywb_expanded,sheet_name,cols=1:ncol(to_export_expanded[["סהכ"]]),widths = "auto")
},names(mywb_expanded))
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
stle_per <- createStyle(numFmt = "0.0%")
#replace default styles of cells
addStyle(mywb_expanded,"סהכ",style = style_num,rows = 2:(nrow(to_export_expanded[["סהכ"]])+2),cols=2:ncol(to_export_expanded[["סהכ"]]),gridExpand = T)
addStyle(mywb_expanded,"שיעורים",style = stle_per,rows = 2:(nrow(to_export_expanded[["סהכ"]])+2),cols=2:ncol(to_export_expanded[["סהכ"]]),gridExpand = T)
addStyle(mywb_expanded,"תצפיות",style = style_num,rows = 2:(nrow(to_export_expanded[["סהכ"]])+2),cols=2:ncol(to_export_expanded[["סהכ"]]),gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_expanded,paste0(OUTPUT,paste("saka21_expanded_25_64_",ISSUE_DATE,".xlsx")),overwrite = T)

#monthly analysis 15+-----------------------------------------------------------------------------------------------------

#create the result output
results_15_plus <- monthly_regular_analysis(wrk_df,min_age=15,max_age=100)
#bind the list into one dataframe
results_15_plus <- bind_rows(results_15_plus) %>% rows_order

#split the big DF to different tables by their columns, later they will be different excel sheets
to_export_15_plus <- list()
to_export_15_plus[["סהכ"]] <- results_15_plus %>%
  select(starts_with(c("population_group","population_total"))) %>% 
  group_names

to_export_15_plus[["שיעורים"]] <- results_15_plus %>%
  select(contains(c("population_group","ratio"))) %>% 
  group_names

to_export_15_plus[["תצפיות"]] <- results_15_plus %>%
  select(starts_with(c("population_group","obs"))) %>% 
  group_names

#replace column names to hebrew
names(to_export_15_plus[["סהכ"]]) <- gen_col_names()
names(to_export_15_plus[["תצפיות"]]) <- gen_col_names()
names(to_export_15_plus[["שיעורים"]]) <- gen_col_names_ratio()

#create the excel file
#add worksheet and assign data for each item in to_export_15_plus_expanded list
mywb_15_plus <- createWorkbook()
Map(function(dat,dat_name){
  addWorksheet(mywb_15_plus,dat_name)
  writeData(mywb_15_plus,dat_name,dat,startRow = 2)
},to_export_15_plus,names(to_export_15_plus))
#freeze panes and adjust column width for each sheet
Map(function(sheet_name){
  freezePane(mywb_15_plus,sheet_name,firstCol = T)
  setColWidths(mywb_15_plus,sheet_name,cols=1:ncol(to_export_15_plus[["סהכ"]]),widths = "auto")
},names(mywb_15_plus))
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
stle_per <- createStyle(numFmt = "0.0%")
#replace default styles of cells
addStyle(mywb_15_plus,"סהכ",style = style_num,rows = 2:(nrow(to_export_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_15_plus[["סהכ"]]),gridExpand = T)
addStyle(mywb_15_plus,"שיעורים",style = stle_per,rows = 2:(nrow(to_export_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_15_plus[["סהכ"]]),gridExpand = T)
addStyle(mywb_15_plus,"תצפיות",style = style_num,rows = 2:(nrow(to_export_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_15_plus[["סהכ"]]),gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_15_plus,paste0(OUTPUT,paste("saka21_general_15_plus_",ISSUE_DATE,".xlsx")),overwrite = T)

# monthly analysis - other files ---------------------------------------------------

# 1) ratio of not working not learning ages 18-24, male, by population
# halat counts as unemployed
def_not_work_not_study <- function(data){
  data %>% 
    mutate(workforce_attr=case_when(
      muasak==1 ~ "employed",
      muasak==2 ~ "unemployed",
      is.na(muasak) ~ "out_of_workforce")) %>% 
    mutate(corona_workforce_attr=case_when(
      corona_missing==1 ~ "halat",
      muasak==1 ~ "employed",
      muasak==2 ~ "unemployed",
      is.na(muasak) ~ "out_of_workforce")) %>% 
    mutate(workforce_new=case_when(
      shnatseker>2019~corona_workforce_attr,
      T~workforce_attr)) %>% 
    mutate(not_work_not_study= case_when(
      limudimveavoda==5~1,
      (limudimveavoda==2) & (workforce_new=="halat")~1,
      T~0))
}

adj_df <- wrk_df %>% def_not_work_not_study()

not_work_not_study_18_24 <- adj_df %>% 
  filter(gil>=18&gil<=24&shnatseker==2021) %>%
  group_by(shnatseker,chodeshseker,not_work_not_study,group,sex) %>%
  summarise(population=sum(mishkalsofi),obs=n()) %>% 
  filter(obs>=5)%>% 
  pivot_wider(names_from=c(not_work_not_study),values_from=c(population,obs)) %>%
  mutate(all=(population_1+population_0),
         rate=(population_1/all)) %>%
  mutate(group = case_when(
    group =="Arab"~"ערבים",
    group == "Haredi"~"חרדים",
    group == "Jew_non_Haredi (including Other)"~"יהודים שאינם חרדים"),
    sex = case_when(
      sex == "female"~"נשים",
      sex == "male"~ "גברים"))

#a bit of formmating of the output, adding commas and "%" to numbers.
mywb_not_work_not_learning <- createWorkbook()
addWorksheet(mywb_not_work_not_learning,"sheet1")
writeData(mywb_not_work_not_learning,"sheet1",not_work_not_study_18_24,startRow = 3)
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
stle_per <- createStyle(numFmt = "0.0%")
#replace default styles of cells
addStyle(mywb_not_work_not_learning,"sheet1",style = style_num,rows = 3:(nrow(not_work_not_study_18_24)+3),cols=5:9,gridExpand = T)
addStyle(mywb_not_work_not_learning,"sheet1",style = stle_per,rows = 3:(nrow(not_work_not_study_18_24)+3),cols=10,gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_not_work_not_learning,paste0(OUTPUT,paste("not_work_not_study_18_24_",ISSUE_DATE,".xlsx")),overwrite = T)

# 2) monthly employed per anaf 15+

employed_per_anaf_monthly <- sum_basic_month_and_year(wrk_df,anafkalkalink,min_year = MAX_YEAR,min_age = 15,max_age = 100) %>% 
  select(contains(c("population_group","total_employed","obs_employed"))) %>% 
  filter(population_group!="")
names(employed_per_anaf_monthly) <- gen_col_names_anaf()

#a bit of formmating of the output, adding commas to numbers.
mywb_employed_per_anaf_monthly <- createWorkbook()
addWorksheet(mywb_employed_per_anaf_monthly,"sheet1")
writeData(mywb_employed_per_anaf_monthly,"sheet1",employed_per_anaf_monthly,startRow = 2)
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
#replace default styles of cells
addStyle(mywb_employed_per_anaf_monthly,"sheet1",style = style_num,rows = 2:(nrow(employed_per_anaf_monthly)+2),cols=2:ncol(employed_per_anaf_monthly),gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_employed_per_anaf_monthly,paste0(OUTPUT,paste("saka21_employed_per_anaf_15_plus_",ISSUE_DATE,".xlsx")),overwrite = T)

# 3) amount of unemployed and mityaashim by age groups and highest diploma

#similar to the monthly analysis, i separate to two tables, one for total sums and one for obs
# then i translate column names to hebrew
# by highest diploma
results_presentation <- list()
results_presentation[["by_education_total"]] <- sum_basic_month_and_year(wrk_df,workforce = workforce_attr_inc_mit,teudagvoha,min_year = MAX_YEAR) %>% select(contains(c("population_group","population_total","obs"))) %>%  filter(!(population_group %in% c(0,7,8,9,99))) %>% group_names_education()
results_presentation[["by_education_obs"]] <- results_presentation[["by_education_total"]] %>% select(contains((c("population_group","obs"))))
names(results_presentation[["by_education_obs"]]) <- gen_col_names_mit()
results_presentation[["by_education_total"]] <- results_presentation[["by_education_total"]] %>% select(contains((c("population_group","population_total"))))
names(results_presentation[["by_education_total"]]) <- gen_col_names_mit()

# by age groups
results_presentation[["by_age_group_total"]] <- sum_basic_month_and_year(wrk_df,workforce = workforce_attr_inc_mit,age_group_wide,min_year = MAX_YEAR,min_age = 15,max_age = 100) %>%  filter(population_group!="other")
results_presentation[["by_age_group_obs"]] <- results_presentation[["by_age_group_total"]] %>% select(contains((c("population_group","obs"))))
names(results_presentation[["by_age_group_obs"]]) <- gen_col_names_mit()
results_presentation[["by_age_group_total"]] <- results_presentation[["by_age_group_total"]] %>% select(contains((c("population_group","population_total"))))
names(results_presentation[["by_age_group_total"]]) <- gen_col_names_mit()

#adding formating to that i could place the tables (for each type of analysis) on top of each other
mywb_results_presentation <- createWorkbook()
addWorksheet(mywb_results_presentation,"לפי תעודה אחרונה")
writeData(mywb_results_presentation,"לפי תעודה אחרונה",results_presentation[["by_education_total"]],startRow = 3)
writeData(mywb_results_presentation,"לפי תעודה אחרונה",results_presentation[["by_education_obs"]],startRow = 14)
addWorksheet(mywb_results_presentation,"לפי קבוצות גיל")
writeData(mywb_results_presentation,"לפי קבוצות גיל",results_presentation[["by_age_group_total"]],startRow = 3)
writeData(mywb_results_presentation,"לפי קבוצות גיל",results_presentation[["by_age_group_obs"]],startRow = 14)
style_num <- createStyle(numFmt = "COMMA")
addStyle(mywb_results_presentation,"לפי תעודה אחרונה",style = style_num,rows = 4:20,cols=2:ncol(results_presentation[["by_education_total"]]),gridExpand = T)
addStyle(mywb_results_presentation,"לפי קבוצות גיל",style = style_num,4:19,cols=2:ncol(results_presentation[["by_age_group_total"]]),gridExpand = T)
saveWorkbook(mywb_results_presentation,paste0(OUTPUT,paste("saka21_unemployed_and_mityaesh_",ISSUE_DATE,".xlsx")),overwrite = T)

# yearly analysis 25-64 ---------------------------------------------------

#create the result output
results_yearly_25_64 <- yearly_regular_analysis(wrk_df,min_year = MAX_YEAR)
#bind the list into one dataframe
results_yearly_25_64 <- bind_rows(results_yearly_25_64)

#split the big DF to different tables by their columns, later they will be different excel sheets

to_export_yearly_25_64[["סהכ"]] <- results_yearly_25_64 %>%
  select(starts_with(c("population_group","population_total"))) %>% 
  group_names

to_export_yearly_25_64[["שיעורים"]] <- results_yearly_25_64 %>%
  select(contains(c("population_group","ratio"))) %>% 
  group_names

to_export_yearly_25_64[["תצפיות"]] <- results_yearly_25_64 %>%
  select(starts_with(c("population_group","obs"))) %>% 
  group_names

#replace column names to hebrew
names(to_export_yearly_25_64[["סהכ"]]) <- gen_col_names_yearly()
names(to_export_yearly_25_64[["תצפיות"]]) <- gen_col_names_yearly()
names(to_export_yearly_25_64[["שיעורים"]]) <- gen_col_names_ratio_yearly()

#create the excel file
#add worksheet and assign data for each item in to_export_yearly_25_64_expanded list
mywb_general_yearly <- createWorkbook()
Map(function(dat,dat_name){
  addWorksheet(mywb_general_yearly,dat_name)
  writeData(mywb_general_yearly,dat_name,dat,startRow = 2)
},to_export_yearly_25_64,names(to_export_yearly_25_64))
#freeze panes and adjust column width for each sheet
Map(function(sheet_name){
  freezePane(mywb_general_yearly,sheet_name,firstCol = T)
  setColWidths(mywb_general_yearly,sheet_name,cols=1:ncol(to_export_yearly_25_64[["סהכ"]]),widths = "auto")
},names(mywb_general_yearly))
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
stle_per <- createStyle(numFmt = "0.0%")
#replace default styles of cells
addStyle(mywb_general_yearly,"סהכ",style = style_num,rows = 2:(nrow(to_export_yearly_25_64[["סהכ"]])+2),cols=2:ncol(to_export_yearly_25_64[["סהכ"]]),gridExpand = T)
addStyle(mywb_general_yearly,"שיעורים",style = stle_per,rows = 2:(nrow(to_export_yearly_25_64[["סהכ"]])+2),cols=2:ncol(to_export_yearly_25_64[["סהכ"]]),gridExpand = T)
addStyle(mywb_general_yearly,"תצפיות",style = style_num,rows = 2:(nrow(to_export_yearly_25_64[["סהכ"]])+2),cols=2:ncol(to_export_yearly_25_64[["סהכ"]]),gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_general_yearly,paste0(OUTPUT,paste("saka21_general_yearly_25_64_",ISSUE_DATE,".xlsx")),overwrite = T)

# yearly analysis 15+ ---------------------------------------------------

#create the result output
results_yearly_15_plus <- yearly_regular_analysis(wrk_df,min_year = MAX_YEAR,min_age=15,max_age=100)
#bind the list into one dataframe
results_yearly_15_plus <- bind_rows(results_yearly_15_plus)

#split the big DF to different tables by their columns, later they will be different excel sheets

to_export_yearly_15_plus[["סהכ"]] <- results_yearly_15_plus %>%
  select(starts_with(c("population_group","population_total"))) %>% 
  group_names

to_export_yearly_15_plus[["שיעורים"]] <- results_yearly_15_plus %>%
  select(contains(c("population_group","ratio"))) %>% 
  group_names

to_export_yearly_15_plus[["תצפיות"]] <- results_yearly_15_plus %>%
  select(starts_with(c("population_group","obs"))) %>% 
  group_names

#replace column names to hebrew
names(to_export_yearly_15_plus[["סהכ"]]) <- gen_col_names_yearly()
names(to_export_yearly_15_plus[["תצפיות"]]) <- gen_col_names_yearly()
names(to_export_yearly_15_plus[["שיעורים"]]) <- gen_col_names_ratio_yearly()

#create the excel file
#add worksheet and assign data for each item in to_export_yearly_15_plus_expanded list
mywb_yearly_15_plus <- createWorkbook()
Map(function(dat,dat_name){
  addWorksheet(mywb_yearly_15_plus,dat_name)
  writeData(mywb_yearly_15_plus,dat_name,dat,startRow = 2)
},to_export_yearly_15_plus,names(to_export_yearly_15_plus))
#freeze panes and adjust column width for each sheet
Map(function(sheet_name){
  freezePane(mywb_yearly_15_plus,sheet_name,firstCol = T)
  setColWidths(mywb_yearly_15_plus,sheet_name,cols=1:ncol(to_export_yearly_15_plus[["סהכ"]]),widths = "auto")
},names(mywb_yearly_15_plus))
#create specific styles
style_num <- createStyle(numFmt = "COMMA")
stle_per <- createStyle(numFmt = "0.0%")
#replace default styles of cells
addStyle(mywb_yearly_15_plus,"סהכ",style = style_num,rows = 2:(nrow(to_export_yearly_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_yearly_15_plus[["סהכ"]]),gridExpand = T)
addStyle(mywb_yearly_15_plus,"שיעורים",style = stle_per,rows = 2:(nrow(to_export_yearly_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_yearly_15_plus[["סהכ"]]),gridExpand = T)
addStyle(mywb_yearly_15_plus,"תצפיות",style = style_num,rows = 2:(nrow(to_export_yearly_15_plus[["סהכ"]])+2),cols=2:ncol(to_export_yearly_15_plus[["סהכ"]]),gridExpand = T)
#the paste0 allows to add elements to file name, such as output (for saving it in a different location) and current date
saveWorkbook(mywb_yearly_15_plus,paste0(OUTPUT,paste("saka21_general_yearly_15_plus_",ISSUE_DATE,".xlsx")),overwrite = T)
